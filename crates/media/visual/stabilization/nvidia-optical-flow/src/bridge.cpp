...
extern "C" {
    Context* shrimply_nvof_create(CUcontext cuda_context, CUstream stream, uint32_t width, uint32_t height, uint32_t quality, uint32_t output_grid, char* error, size_t error_size) { return NULL; }
    int shrimply_nvof_estimate(Context* context, CUdeviceptr input, CUdeviceptr reference, int use_temporal_hints, int disable_temporal_hints, NV_OF_FLOW_VECTOR* forward, NV_OF_FLOW_VECTOR* backward, uint8_t* forward_cost, uint8_t* backward_cost, char* error, size_t error_size) { return -1; }
    void shrimply_nvof_destroy(Context* context) {}
}
#else
#include <cuda.h>
#include <dlfcn.h>
